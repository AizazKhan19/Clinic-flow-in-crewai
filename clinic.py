...
class MyClinicStates(BaseModel):
    ...
    is_registered : bool = False      # Becomes True when new patient is registered or found old one
    current_symptoms : str = ""       # All symptoms will be stored here
    symptoms_complete : bool = False  # Becomes True when no need to ask more symptoms/follow-up questions
    
    # Router Key

    doctor_type : str = ""            # A string such as cardiologist, orthopedic, general will be store here and will help for routing

    # Doctor Decision states global
             
    prescription : str = ""            # Prescription by doctor stored here if symptoms seems mild
    required_tests : str = ""          # Required tests will be stored here when symptoms seems critical
    is_critical : bool = False         # Becomes True when symtpoms seeems critical else False
    

    # Lab testting state  

    report : str = ""                  # Simulated report generated by lab agent will be stored here
    is_report_ready : bool = False     # set to True when report is simulated/Generated
...
class ClinicExtractionSchema(BaseModel):
    ...
    tool_called_for_save: bool = Field(False, description="Set to True ONLY if you actually called CSVReadWriteTool and received a success response with a patient ID")
        ...
        while not self.state.symptoms_complete:
            ...
            desk_task = Task(
                ...
                2. REGISTRATION: If ID not found then:
                    ...
                    i.  (VERY IMPORTANT FOR SUCCESSFUL REGISTRATION) : Collect mandatory information including 'Name', 'Age', 'Gender', 'Contact',
                    ...
                    REGISTRATION. (VERY IMPORTANT) and you must ask user to provide the missing things.
